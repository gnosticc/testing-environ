# crossbowbolt.gd
# CORRECTED: This version uses the standardized `set_attack_properties` function
# to correctly receive all data from the WeaponManager.
extends Area2D

var final_damage_amount: int = 18
var final_speed: float = 200.0
var final_applied_scale: Vector2 = Vector2(1,1)
var direction: Vector2 = Vector2.RIGHT # Default value, will be overridden
...
func _ready():
	if not is_instance_valid(lifetime_timer):
		print("ERROR (CrossbowBolt): LifetimeTimer missing!"); call_deferred("queue_free"); return
	else:
		if not lifetime_timer.is_connected("timeout", Callable(self, "queue_free")):
			lifetime_timer.timeout.connect(self.queue_free)
	
	# REMOVED: Initial rotation from _ready(). Rotation will now be set in set_attack_properties.
...
func set_attack_properties(p_direction: Vector2, p_attack_stats: Dictionary, p_player_stats: PlayerStats):
	...
	# ADDED: Set the rotation here, AFTER the direction is confirmed.
