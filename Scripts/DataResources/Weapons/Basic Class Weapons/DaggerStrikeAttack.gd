...
# Behavior for the Dagger Strike attack.
# A fast, aimed melee attack.
...
var _current_attack_duration: float = 0.25 

func _ready():
	if not is_instance_valid(animated_sprite):
		print("ERROR (DaggerStrikeAttack): AnimatedSprite2D node missing."); call_deferred("queue_free"); return
	else:
		animated_sprite.animation_finished.connect(Callable(self, "_on_animation_finished"))

	if not is_instance_valid(damage_area):
		print_debug("WARNING (DaggerStrikeAttack): DamageArea node missing.")
	else:
		...
		if is_instance_valid(collision_shape):
			...
			print_debug("WARNING (DaggerStrikeAttack): No CollisionShape2D found under DamageArea.")
...
func _on_damage_area_body_entered(body: Node2D):
	...
	if body.is_in_group("enemies") and body is BaseEnemy:
		...
		var weapon_damage_percent = float(specific_stats.get("weapon_damage_percentage", 0.9)) # 90% base
