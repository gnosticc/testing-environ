# File: res://Scripts/Weapons/Advanced/Turrets/HunterKillerProjectile.gd
class_name HunterKillerProjectile
extends Area2D
...
func initialize(p_target: BaseEnemy, p_stats: Dictionary, p_player_stats: PlayerStats):
	...
	var base_scale = float(p_stats.get("hk_projectile_scale", 1.0))
	self.scale = Vector2.ONE * base_scale
	
	speed = float(p_stats.get("hk_projectile_speed", 300.0)) * p_player_stats.get_final_stat(PlayerStatKeys.Keys.PROJECTILE_SPEED_MULTIPLIER)
	
	var damage_percent = float(p_stats.get("hk_damage_percent", 3.0)) * float(p_stats.get("turret_damage_mult", 1.0))
